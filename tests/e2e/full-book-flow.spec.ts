...
test("uploads, analyzes, translates, and exposes the EPUB download",async({page})=>{
  ...
  const configRequest=page.waitForRequest(request=>request.url().includes("/api/jobs/")&&request.url().endsWith("/config")&&request.method()==="PUT");
  await page.getByRole("button",{name:"Upload and analyze"}).click();
  expect(JSON.parse((await configRequest).postData() ?? "{}").glossary).toEqual([expect.objectContaining({source:"Cthulhu",target:"Ктулху",category:"proper name",enabled:true})]);
