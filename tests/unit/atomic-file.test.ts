...
const roots:string[]=[];
async function temporaryRoot():Promise<string>{const root=await mkdtemp(join(tmpdir(),"atomic-file-"));roots.push(root);return root;}
afterEach(async()=>{await Promise.all(roots.splice(0).map(root=>rm(root,{recursive:true,force:true})));});

describe("atomicWrite",()=>{
  it("replaces the destination and leaves no temporary file",async()=>{
    const root=await temporaryRoot(),destination=join(root,"state.json");
