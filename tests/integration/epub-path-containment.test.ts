...
describe("EPUB path containment", () => {
  it.each([
    ...
    "/etc/passwd",
    "C:/secret.xhtml",
    "https://example.test/book.xhtml",
    "//server/share",
    "chapter.xhtml#part",
    "chapter.xhtml?raw=1",
    "chapter%2fxhtml",
    "chapter%5cxhtml",
    "%2e%2e%2foutside.xhtml",
    "chapter\\name.xhtml",
  ...
  it("resolves a percent-encoded file name inside staging", async () => {
    const root = await temporaryRoot();
    expect(resolveEpubPath(root, "Text/chapter%201.xhtml", "OEBPS")).toBe(
      join(root, "OEBPS/Text/chapter 1.xhtml"),
    );
  });
  ...
  it("rejects a container package path that escapes staging", async () => {
    ...
    await writeEpub(join(root, "source.epub"), {
      "META-INF/container.xml": `<container><rootfiles><rootfile full-path="../outside.opf"/></rootfiles></container>`,
    });
