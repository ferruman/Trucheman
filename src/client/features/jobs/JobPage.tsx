...
export function JobPage({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToJobEvents(id, {
      onEvent: (event) => {
        setEvents((current) =>
          current.some((item) => item.id === event.id) ? current : [...current, event].slice(-250),
        );
        void refresh(controller.signal);
      },
    ...
    <section className="page job-page">
      ...
      <div className="job-workbench">
        <div className="job-primary">
          ...
          {loadError && (
            <div role="alert">
              <p>Latest status could not be refreshed: {loadError}</p>
        ...
        <aside className="job-transcript" aria-labelledby="snapshot-heading">
          ...
          <dl className="state-snapshot">
            ...
            <div>
              <dt>Document</dt>
              <dd>
                <code>{job.currentDocument ?? "No active document"}</code>
              </dd>
            </div>
            <div>
              <dt>Warnings</dt>
              <dd className={job.warnings > 0 ? "warning" : ""}>{job.warnings}</dd>
            </div>
          </dl>
          <div className="connection-state">
            <span className={connected ? "status-light" : "status-light offline"} />
            {connected ? "Live updates connected" : "Polling fallback active"}
          </div>
        </aside>
      </div>
      <JobLogPanel events={events} />
      <InvalidationDialog
