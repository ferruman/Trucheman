...
export function ResultPage({ id, results, error, busy, onRetry, onRebuild }: Props) {
  return <section className="result-panel" aria-labelledby="result-heading">
    <div className="panel-heading"><span className="section-label">Output</span><h2 id="result-heading">Translated EPUB ready</h2></div>
    ...
    {results && <>
      <dl className="result-checks">
        <div><dt>Validation report</dt><dd>{results.validation === null ? "Not available" : "Available"}</dd></div>
        <div><dt>Translation statistics</dt><dd>{results.statistics === null ? "Not available" : "Available"}</dd></div>
      </dl>
