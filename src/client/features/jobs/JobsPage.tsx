...
export function JobsPage() {
  ...
  return <section className="page jobs-page">
    <header className="page-header">
      <div><span className="section-label">Local queue</span><h1>Translation jobs</h1></div>
      <a className="button-link" href="/new">New book</a>
    </header>
    ...
    {jobs?.length === 0 && !error && <div className="empty-state">
      ...
      <a className="button-link" href="/new">Create a job</a>
    </div>}
    {jobs && jobs.length > 0 && <div className="job-list-wrap"><div className="job-list-head" aria-hidden="true">
      <span>Book</span><span>Languages</span><span>Status</span><span>Updated</span><span>Actions</span>
    </div><ul className="job-list">{jobs.map(job => <li key={job.id}>
      <a href={`/jobs/${job.id}`}>
        <span className="job-title">{job.title}<small>{job.id}</small></span>
        <span className="language-pair">{job.sourceLanguage.toUpperCase()} <b aria-hidden="true">→</b> {job.targetLanguage.toUpperCase()}</span>
      ...
      <button className="danger job-delete" type="button" onClick={() => setJobToDelete(job)}>Delete</button>
    </li>)}</ul></div>}
    <DeleteJobDialog open={Boolean(jobToDelete)} busy={deleting} onCancel={() => setJobToDelete(undefined)} onConfirm={() => void deleteJob()}/>
  </section>;
}
