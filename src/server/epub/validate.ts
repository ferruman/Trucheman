...
export type ValidationReport={ok:boolean;errors:string[];warnings:string[];documents:number};
...
export function resolveEpubPath(root:string,reference:string,baseDirectory=""):string{
  if(!reference||reference!==reference.trim()||reference.includes("\\")||reference.includes("\0")||reference.includes("?")||reference.includes("#")||encodedSeparator.test(reference))throw new Error(`Unsafe EPUB path: ${reference}`);
  let decoded:string;
  try{decoded=decodeURIComponent(reference);}catch{throw new Error(`Invalid encoded EPUB path: ${reference}`);}
  if(!decoded||decoded.includes("\\")||decoded.includes("\0")||decoded.includes("?")||decoded.includes("#")||encodedSeparator.test(decoded)||posix.isAbsolute(decoded)||decoded.startsWith("//")||uriScheme.test(decoded)||/^[A-Za-z]:/.test(decoded))throw new Error(`Unsafe EPUB path: ${reference}`);
  const relativePath=posix.normalize(posix.join(baseDirectory,decoded));
  if(relativePath==="."||relativePath===".."||relativePath.startsWith("../"))throw new Error(`EPUB path escapes its root: ${reference}`);
...
export async function validateEpub(root:string):Promise<ValidationReport>{
  ...
  try{
    ...
    if((await readFile(join(root,"mimetype"),"utf8"))!=="application/epub+zip")errors.push("Invalid mimetype");
    ...
    const pkg=await readFile(packageFile,"utf8"),book=parsePackage(pkg,packagePath);
    for(const id of book.spine){
      const item=book.manifest.get(id);
      if(!item){errors.push(`Missing spine item ${id}`);continue;}
      try{
        const resource=resolveEpubPath(root,item.href,posix.dirname(packagePath));
        parseXml(await readFile(resource));
        documents++;
      }catch{errors.push(`Invalid XML or unsafe path ${item.href}`);}
    }
  }catch(e){errors.push(e instanceof Error?e.message:"Unable to validate EPUB");}
...
export async function validateEpubArchive(archivePath:string):Promise<ValidationReport>{
  ...
  try{
    ...
    return {ok:false,errors:[e instanceof Error?e.message:"Unable to validate EPUB archive"],warnings:[],documents:0};
  }finally{
