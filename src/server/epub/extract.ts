...
export async function extractEpub(source: string, destination: string): Promise<string[]> {
  ...
  const names = validateArchiveEntries(entries.map((entry) => ({ fileName: entry.fileName, compressedSize: entry.compressedSize, uncompressedSize: entry.uncompressedSize, compressionMethod: entry.compressionMethod, encrypted: entry.isEncrypted() })), defaultArchiveLimits);
  for (let i = 0; i < entries.length; i++) {
    const entry = entries[i], name = names[i], target = join(destination, name);
...
export async function cleanupExtractedJob(path: string) { await rm(path, { recursive: true, force: true }); }
