...
export function createApp(dataDir:string){
  const app=express(),jobs=new JobRepository(dataDir),settings=new SettingsRepository(join(dataDir,"settings.json")),events=new EventRepository(join(dataDir,"events.ndjson"));
  app.disable("x-powered-by");
  app.use(express.raw({type:["application/epub+zip","application/octet-stream"],limit:"100mb"}));
  app.use(express.json({limit:"1mb"}));
  app.use("/api/jobs",jobsRouter(jobs));
  app.use("/api/settings",settingsRouter(settings));
  app.use("/api/jobs",jobEventsRouter(events));
  app.use("/api/jobs",jobControlRouter());
  app.use("/api/jobs",jobRetryRouter());
  app.use("/api/jobs",jobResultsRouter());
  ...
  return {app,jobs,settings,events};
}
