...
│   ├── languages.ts          # Supported language definitions (en, ru, de, pl, ja)
...
└── server/          # Node.js Express backend
    ...
    ├── languages/            # Per-language capabilities and directional pair rules
...
**Source-language normalization** is selected through `languages/registry.ts`. The Japanese module delegates its EPUB operations to `epub/japanese.ts` and runs them inside `prepareBook`, over `staging/` and before anything is segmented, so reinsertion and assembly need to know nothing about them. It undoes what belongs to the original rather than to the translation: `page-progression-direction="rtl"` becomes `ltr`, the `vrtl` class on each content document becomes `hltr`, Japanese-only font stacks are removed, and ruby is flattened to its base. Flattening **must** merge the resulting adjacent text nodes: left split, a document segments one way in memory and another way when assembly reads the file back, and reinsertion fails the whole book with "Source changed". The furigana is not discarded but harvested into `PreparedBook.readings` and handed to the entity registry, where it is what makes a name transliterable at all.

Language-specific behavior is capability-based. `languages/<tag>.ts` owns source preprocessing, batch limits, target prompt/style rules, deterministic normalization, written-number recognition, EPUB audits, and optional morphology. `languages/pairs/<source>-<target>.ts` owns directional rules such as Polivanov for `ja→ru`. `batcher.ts`, `book-pipeline.ts`, `segment-scan.ts`, prompts, consistency, and EPUB audit ask the registry for capabilities instead of branching on tags. See [docs/language-modules.md](docs/language-modules.md) for the extension contract. Script-derived heuristics in `response-validator.ts` and entity extraction remain generic because they inspect each actual segment rather than assume the configured language.
