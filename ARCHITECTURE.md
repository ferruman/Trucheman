...
The provider is `DeepSeekProvider` only when `TRUCHEMAN_PROVIDER !== "deterministic"` **and** both the translation and editing keys exist; otherwise `FakeProvider` (a deterministic `[translated] …` prefix), which is what the tests and the e2e suite run against. Profiles come from `config/profiles.ts` — translation, editing, critic, repair and consistency, each with its own endpoint, model, thinking mode and prompt version, consistency falling back to the translation profile's key. Repair is the exception: only its model and thinking mode are its own (`TRUCHEMAN_REPAIR_MODEL`, `TRUCHEMAN_REPAIR_THINKING`), and it takes the editing endpoint, key and prompt version, because the reason it is separate is a bigger model on the same provider — a strong critic in front of a weak repairer returns a sixth of its repairs unchanged. `resolveProfiles()` is the single source of truth: the pipeline runs on it and `GET /api/settings` reports it, so the UI cannot show configuration a run would not use.

1. **Prepare book** (`prepareBook`): re-extract the EPUB to `staging/` on **every** run — assembly mutates staged documents in place — normalize the staged markup for the source language, parse all XHTML/NCX documents, extract text segments, merge them into logical blocks, batch them
...
5. **Build output EPUB**: reinsert edited text into the DOM, rewrite language tags, build to a `.tmp`, fsync, validate, audit, then rename into `output.epub`. EPUBCheck runs afterwards when installed and is report-only — its errors become job warnings and `epubcheck.txt`, never a failure. The audit's Russian agreement check (`epub/morphology.ts`) is optional in the same way: it needs the `az` package, is skipped silently without it, and only reports — roughly one finding in three is real, well below what routing into repair would require
6. Results available via GET `/:id/download`

**Source-language normalization** (`epub/japanese.ts`, Japanese only) runs inside `prepareBook`, over `staging/` and before anything is segmented, so reinsertion and assembly need to know nothing about it. It undoes what belongs to the original rather than to the translation: `page-progression-direction="rtl"` becomes `ltr`, the `vrtl` class on each content document becomes `hltr` (EBPAJ stylesheets already carry the rule, and a book whose stylesheet does not falls back to horizontal anyway), and ruby is flattened to its base. Flattening **must** merge the resulting adjacent text nodes: left split, a document segments one way in memory and another way when assembly reads the file back, and reinsertion fails the whole book with "Source changed". The furigana is not discarded but harvested into `PreparedBook.readings` and handed to the entity registry, where it is what makes a name transliterable at all.

The rules that vary by source language rather than target are `config/source-language.ts` (prompt rules, Polivanov for ja→ru), `batchCharBudget` and `batchSegmentCap` in `batcher.ts` (a Japanese character costs about a token and says what three Latin ones do, so the budget is smaller — and for Japanese it is the segment cap that binds, since a volume filled 255 of 281 batches to twenty segments at a median of 920 characters, and twenty fragments that short in one answer is what made the model lose count), `expectedExpansion` in `response-validator.ts`, used by both the run-on detector there and the length-ratio corridor in `segment-scan.ts` (both centre on the expansion the script predicts rather than on 1.0 — the second one only reports, but the first _rejects the answer and pays for another_, and left uncorrected it threw away some sixty per cent of a Japanese book's translation requests), and the Japanese candidate patterns in `consistency-service.ts` (Japanese has neither capitals nor spaces, so the capitalization-based extraction finds nothing and the registry silently resolves an empty book). None of this changes a prompt for the languages that came before it, and `checkpointKey` hashes `sourceLanguage`, so no existing checkpoint is invalidated by adding a source language.
...
Prompts are assembled in `providers/prompts.ts` from composable blocks: common rules, a strict JSON output contract, per-mode rules, and `TARGET_LANGUAGE_RULES` keyed by target language. The output contract comes in three shapes, one per answer type — prose in `text`, audit findings in `issues`, a consistency decision as an object in `text` — because a model asked to escape a JSON document inside a JSON string breaks it an order of magnitude more often than it breaks anything else. Book content is explicitly framed as untrusted data, never instructions. `response-validator.ts` enforces the contract (exact ids, exact count, string `text`) and, for translation and editing, that no answer covers **more of the book than its segment does** — a model that glues two halves of a split sentence under the first id returns perfect ids while either shifting every later answer back by one or publishing the glued half twice, so `misalignedSegmentIds` compares each answer's length with its own input, with the next one, and with the two together. `retry-policy.ts` decides what is retried from `ProviderError.kind`:
...
Provider selection is configured via `TRUCHEMAN_PROVIDER` env var and separate profile env vars for translation, editing, critic, and consistency (each with its own endpoint/model/key), plus `TRUCHEMAN_REPAIR_MODEL` for the repair stage alone. The legacy `BOOK_TRANSLATOR_*` prefix remains supported for existing local installations.
...
Model behaviour is measured, not assumed: `npm run report:models` reads every finished job's `usage-report.json` and `quality-report.json` and reports cost and quality per model and stage. The optional literary evaluation tooling accepts only an explicit user-supplied `--corpus`; Trucheman does not bundle book text for evaluation.
